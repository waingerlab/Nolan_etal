...
function [positivecall,callmask] = positive_caller_v2(inputimage,objectmask,backgroundmask,threshold)


%start of function
stdev = double(immultiply(backgroundmask,inputimage));
stdev=stdev(:);
stdev(stdev>60000)=[];%get rid of noise from falsely included background
stdev = std(stdev);
